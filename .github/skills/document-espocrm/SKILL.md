...
7. **Document the instance, not your method.** The reader wants to know what the system *is*, not how you found out. No appendix, no command log, no SQL listings, no raw dumps — and equally no methodology scattered through the prose: no "as revealed by `SHOW TABLES`", no `docker exec` snippets, no parenthetical citations of which JSON file a fact came from. State the fact and move on. Figures like record counts and dates belong in the text because they are *findings* ("4 298 records", "last modified 2025-06-30"), not because they are evidence.
...
- [ ] **1.7 Automations** (Advanced Pack / BPM — skip cleanly if not installed). 510 convention: **always use flowcharts, not workflows** — so lead with BPMN and, if any workflows exist, flag them as legacy/migration candidates in Phase 2.
  ...
  - **Workflows** (discouraged) — `workflow` table: `name`, `entity_type`, trigger `type` (`afterRecordCreated`/`afterRecordUpdated`/`afterRecordSaved`/`scheduled`/`manual`), `is_active`, conditions, actions. If present, document them **and** flag for possible migration to a flowchart.
    > ⚠️ **Treat `is_internal = 1` rows as if they do not exist — always query `WHERE is_internal = 0`.** Advanced Pack auto-creates one hidden `workflow` row per BPMN **Timer Start Event** (hook `custom/Espo/Modules/Advanced/Hooks/BpmnFlowchart/CreateWorkflows.php`). They have `name IS NULL`, `type = 'scheduled'`, `is_internal = 1`, a populated `flowchart_id`, and a single `startBpmnProcess` action.
    >
    > **Do not mention them in the output document at all** — no counts, no "of which N are internal", no explanatory footnote. They are extension plumbing, invisible in the admin UI, and nothing the reader can act on. Reporting them either slanders the team (as "legacy workflows to migrate") or, once explained, wastes a paragraph on a non-issue. **Every workflow count, table and statement in the document is a count of hand-built workflows only.**
      ```sql
      -- always scope to hand-built workflows; is_internal = 1 rows never reach the document
      SELECT name, entity_type, type, is_active FROM workflow WHERE deleted = 0 AND is_internal = 0;
      ```
      Run `SELECT is_internal, COUNT(*) FROM workflow WHERE deleted=0 GROUP BY is_internal;` for **your own orientation only** — it tells you whether a surprising row count is plumbing or real — then discard the internal figure.
...
Fill the standard format below. Use mermaid for journey flows, the data-model diagram, and BPMN reconstructions. Keep secrets out. **The document ends at section 10 — do not append a methodology, command log, query list or raw-dump appendix.**
...
- **One word, one meaning, every time.** Pick a term and never vary it for elegance. If you write "entity" once, do not later write "record type", "object" or "scope" for the same thing. Varying vocabulary is good literary style and bad technical writing.
- **Keep the standard EspoCRM vocabulary — do not invent plainer substitutes for it.** The reader works in EspoCRM, sees these words in the interface, and will search the [wiki](https://github.com/rodekruis/EspoCRM-knowledge-base/wiki) and [EspoCRM docs](https://docs.espocrm.com/) with them. Replacing them makes the document *harder* to use, not easier. Use freely, the way the wiki does: **entity, field, record, flowchart, workflow, report, role, team, formula, dynamic logic, layout, scheduled job, Process record, audited, 2FA, API user, dashlet, Entity Manager, admin UI, Field Level, soft-deleted, personal data flag.** Explain a term the first time only if the reader may not have met it: *"An entity is a type of record."*
- **Do simplify the words that are not EspoCRM vocabulary** — the surrounding prose, and terms from databases, security or software engineering that the reader has no reason to know. Replace: *ACL* → the access rules; *default-deny* → EspoCRM gives no access unless a role gives it; *denormalised* → say what the design does; *tenant / multi-tenancy* → each team sees only its own data; *least privilege* → give only the access that is needed; *materialised* → saved in the database; *introspection* → reading the system; *cross-site scripting*, *idempotent*, *referential integrity* → describe the effect instead.
...
SELECT name, entity_type, type, is_active FROM workflow
  WHERE deleted = 0 AND is_internal = 0;                                        -- hand-built only; never document is_internal = 1 rows (see 1.7)
